...
scale_sds <- rep(0.1, p)
acceptance_L <- res_L <- matrix(0.0, M, K)
scale_L <- rep(0.1, K)
#
current_sds <- sds
current_L <- L
...
system.time({
    for (m in seq_len(M)) {
        ...
        # update the elements of the L matrix
        for (i in seq_len(K)) {
            ...
            pr_L[diags2] <- sapply(split(pr_L[upper_tri_ind], upper_tri_spl),
                                   function (x) sqrt(1 - sum(x * x)))
