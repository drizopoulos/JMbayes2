...
void update_betas (field<vec> &betas, mat &res_betas, //mat &acceptance_betas,
 ...
 const field<uvec> &unq_idL*/
  ...
  // new
  mat sum_JXDXJ2(p_HC, p_HC, fill::zeros);
  vec sum_JXDu2(p_HC, fill::zeros);
  ...
  field<mat> D_inv2(patt_count); // new
  for (uword i = 0; i < n_b; ++i) { // i-th patient
    ...
    if (i < patt_count && !ind_RE_patt.at(i).is_empty()) {
      ...
      D_inv2.at(i) = U_patt_inv.t() * U_patt_inv; // mat
    }
    ...
    // new
    mat D_inv_i2 = D_inv2.at(patt_i);
    mat XD_i2 = X_dot_i.t() * D_inv_i2;
    mat XDX_i2 = XD_i2 * X_dot_i;
    sum_JXDu2  += add_zero_rows(XD_i2*u_i, p_HC, ind_FE_i);
    sum_JXDXJ2 += add_zero_colrows(XDX_i2, p_HC, p_HC, ind_FE_i, ind_FE_i);
  }
  ...
  // new
  mat Sigma_12 = inv(prior_Tau_betas_HC + sum_JXDXJ2); // improve via Cholesky decomposition 
  vec mean_12 = Sigma_12 * (prior_Tau_betas_HC * prior_mean_betas_HC + sum_JXDu2);
...
// [[Rcpp::export]]
List D_inv_test (List control, List initial_values, List priors, List model_data) {
