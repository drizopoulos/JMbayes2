...
void update_betas (field<vec> &betas,
  ...
  mat U = L.each_row() % sds.t(); // RE vcov matrix Choesky factorization (upper)
  ...
  for (uword i = 0; i < n_b; ++i) { // i-th patient
    /* I'me assuming that n_b in the total number of patients, including those 
    ...
    if (i < patt_count && !ind_RE_patt.at(i).is_empty()) { 
      ...
      /*
       * To improve: is there a more efficient way to do the operation above? e.g., in R we could do crossprod(chol(Sigma))
       */
    ...
    X_dot_i = X_dot_i.submat(ind_RE_i, ind_FE_i); // mat
  ...
  mat Tau_1 = inv(prior_Tau_betas_HC + sum_JXDXJ); 
  /*
   * To improve: Can I write the Tau_1 in terms of an L matrix? to avoid the use of chol(Tau_1)
   */ 
...
// [[Rcpp::export]]
List update_betas (List model_data, List model_info, List initial_values,
  ...
  /*
   * 1. Calculating the chol(vcov) avoids repeated (unnecessary) chol() calculations for each outcome
