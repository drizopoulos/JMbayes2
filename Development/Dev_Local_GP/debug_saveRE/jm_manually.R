...
Mixed_objects <- list(fm1_exclude, fm2_exclude)
time_var <- "year"
recurrent = FALSE
functional_forms = CR_forms
data_Surv = NULL
...
jm <- function (Surv_object, Mixed_objects, time_var, recurrent = FALSE,
    ...
    mf_FE_dataL <- JMbayes2:::mapply2(JMbayes2:::fix_NAs_fixed, mf_FE_dataL, NAs_FE_dataL, NAs_RE_dataL)
    mf_RE_dataL <- JMbayes2:::mapply2(JMbayes2:::fix_NAs_random, mf_RE_dataL, NAs_RE_dataL, NAs_FE_dataL)
    ...
    X <- JMbayes2:::mapply2(model.matrix.default, terms_FE, mf_FE_dataL)
    Xbar <- lapply(X, colMeans)
    Z <- JMbayes2:::mapply2(model.matrix.default, terms_RE, mf_RE_dataL)
    ...
    Xbar[] <- JMbayes2:::mapply2(JMbayes2:::center_X, Xbar, x_notin_z)
    ...
    functional_forms <- functional_forms[order(match(names(functional_forms),
                                                     respVars_form))]
    functional_forms <- JMbayes2:::mapply2(JMbayes2:::expand_Dexps, functional_forms, respVars_form)
    ###################################################################
    # List of lists
    ...
    Funs_FunForms <- lapply(functional_forms, JMbayes2:::extractFuns_FunForms,
                            data = dataS)
    #####################################################
    ...
    dataS_H <- JMbayes2:::SurvData_HazardModel(split(st, row(st)), dataS, Time_start,
    ...
    attr <- lapply(functional_forms, JMbayes2:::extract_attributes, data = dataS_H)
    eps <- lapply(attr, "[[", 1L)
    ...
    if (length(which_event)) {
        ...
        dataS_h <- JMbayes2:::SurvData_HazardModel(split(Time_right, seq_along(Time_right)),
                                        dataS, Time_start,
                                        paste0(idT, "_", strata), time_var)
        mf <- model.frame.default(terms_Surv_noResp, data = dataS_h)
        W_h <- JMbayes2:::construct_Wmat(terms_Surv_noResp, mf)
