load(file = 'D:/gpapageorgiou/Github_Repos/JMbayes2/Dev_Local/reprod_fits_eta.RData')
...
jm <- function (Surv_object, Mixed_objects, time_var,
  ...
  idL_ind <- JMbayes2:::mapply2(function (x, y) split(x, y), idL_ind, idL)
  ...
  terms_FE <- lapply(Mixed_objects, JMbayes2:::extract_terms, which = "fixed", data = dataL)
  ...
  terms_RE <- lapply(Mixed_objects, JMbayes2:::extract_terms, which = "random", data = dataL)
  ...
  mf_FE_dataL <- mapply(JMbayes2:::fix_NAs_fixed, mf_FE_dataL, NAs_FE_dataL, NAs_RE_dataL,
                        SIMPLIFY = FALSE)
  mf_RE_dataL <- mapply(JMbayes2:::fix_NAs_random, mf_RE_dataL, NAs_RE_dataL, NAs_FE_dataL,
                        SIMPLIFY = FALSE)
  ...
  unq_id <- unique(idL)
  idL <- mapply(JMbayes2:::exclude_NAs, NAs_FE_dataL, NAs_RE_dataL,
  ...
  componentsHC <- JMbayes2:::mapply2(JMbayes2:::create_HC_X2, X, Z, idL)
  ...
  #X_dot <- JMbayes2:::create_X_dot(Xbase, seq_len(nY), unq_idL, nres)
  nfes <- sapply(X, ncol)
  # 'ind_FE' is used in vec2field() to re-create the field of betas
  # from betas_vec
  ind_FE <- split(seq_len(sum(nfes)), rep(seq_along(X), nfes))
  x_in_z_base <- JMbayes2:::mapply2(function (x, y) sort(c(x, y)), x_in_z, baseline)
  ...
  ind_FE_HC <- unlist(JMbayes2:::mapply2(function (x, ind) x[ind], ind_FE, x_in_z_base),
                      use.names = FALSE)
  ...
  ind_FE_nHC <- JMbayes2:::mapply2(function (x, ind) x[-ind], ind_FE, x_in_z_base)
  ...
  GK <- JMbayes2:::gaussKronrod(con$GK_k)
  sk <- GK$sk
  ...
  if (is.null(con$knots)) {
    ...
    con$knots <- JMbayes2:::knots(qs[1L], qs[2L], con$base_hazard_segments,
                       con$Bsplines_degree)
  }
  ...
  FunForms_per_outcome <- lapply(functional_forms, JMbayes2:::extract_functional_forms,
                                 data = dataS)
  ...
  W0_H <- JMbayes2:::create_W0(c(t(st)), con$knots, con$Bsplines_degree + 1, strata_H)
  ...
  X_H <- JMbayes2:::desing_matrices_functional_forms(st, terms_FE_noResp,
                                          dataL, time_var, idVar,
                                          collapsed_functional_forms)
  Z_H <- JMbayes2:::desing_matrices_functional_forms(st, terms_RE,
                                          dataL, time_var, idVar,
                                          collapsed_functional_forms)
  U_H <- lapply(functional_forms, JMbayes2:::construct_Umat, dataS = dataS_H)
  if (length(which_event)) {
    W0_h <- JMbayes2:::create_W0(Time_right, con$knots, con$Bsplines_degree + 1, strata)
    ...
    X_h <- JMbayes2:::desing_matrices_functional_forms(Time_right, terms_FE_noResp,
                                            dataL, time_var, idVar,
                                            collapsed_functional_forms)
    Z_h <- JMbayes2:::desing_matrices_functional_forms(Time_right, terms_RE,
                                            dataL, time_var, idVar,
                                            collapsed_functional_forms)
    U_h <- lapply(functional_forms, JMbayes2:::construct_Umat, dataS = dataS_h)
  } else {
  ...
  init_surv <- JMbayes2:::init_vals_surv(Data, model_info, data, betas, b, con)
  ...
  #    centering part
  vcov_prop_betas <- lapply(Mixed_objects, JMbayes2:::vcov2)
  vcov_prop_betas <- JMbayes2:::mapply2(JMbayes2:::get_betas_nHC, vcov_prop_betas, x_notin_z)
  r <- JMbayes2:::mapply2(JMbayes2:::extract_vcov_prop_RE, Mixed_objects, Z, idL)
  ...
  for (i in seq_len(nY)) {
    ...
    vcov_prop_RE[, , i] <- JMbayes2:::.bdiag(rr)
  }
  ...
  mean_betas_nHC <- JMbayes2:::mapply2(JMbayes2:::get_betas_nHC, mean_betas, x_notin_z)
  Tau_betas_nHC <- JMbayes2:::mapply2(JMbayes2:::get_betas_nHC, Tau_betas, x_notin_z)
