...
We evaluate the accuracy of the predictions using ROC methodology. We calculate the components of the ROC curve using information up to year seven, and we are interested in events occurring within a two-year window. That is discriminating between patients who will get the event in the interval `(t0, t0 + Dt]`, (i.e., in our case $T_j \in (7, 9]$) from patients who will survive at least 9 years (i.e., $T_j > 9$). The calculations are performed with the following call to `tvROC()`:
```{r}
pbc2$event <- as.numeric(pbc2$status != "alive")
roc <- tvROC(jointFit, newdata = pbc2, Tstart = t0, Dt = 2)
roc
```
