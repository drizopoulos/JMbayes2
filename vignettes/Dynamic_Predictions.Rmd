...
We evaluate the accuracy of the predictions using ROC methodology. We calculate the components of the ROC curve using information up to year seven, and we are interested in events occurring within a two-year window. The calculations are performed with the following call to `tvROC()`:
```{r}
...
In the first line we define the event indicator as we did in the `pbc2.id` data.frame. To depict the ROC curve we use the corresponding `plot()` method:
```{r, fig.align = "center", fig.width = 8.5, fig.height = 7.5, echo = FALSE}
plot(roc)
```
