...
$$\mbox{BS}(t + \Delta t, t) = E \left [\left \{ \mathbb{I}(T_i^* \leq t + \Delta t) - \tilde{\pi}_i^{v}(t + \Delta t \mid t) \right \}^2 \; \Big | \; T_i^* > t \right].$$
...
$$\mbox{EPCE}(t + \Delta t, t) = E \left \{-\log \left [ p \bigl \{ T_i^* \mid t < T_i^* < t + \Delta t, \mathcal Y_i(t), \mathcal D_{n} \bigr \} \right ] \right \},$$
...
$$\widehat{\varpi}_l(t) = \mbox{argmin}_{\varpi} \left [ \mathcal{S} \left \{ \sum_{l = 1}^L \varpi_l \hat{\pi}_i^{(v)}(t + \Delta t \mid t, M_l), T_i, \delta_i \right \} \right], \quad v = 1, \ldots, V,$$
