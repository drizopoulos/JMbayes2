...
We observe that the first three models dominate the weights. We also note that the integrated Brier score based on the combined predictions is smaller than the integrated Brier score of each individual model. To calculate the model weights using the expected predictive cross-entropy use function `tvEPCE()` with an almost identical call as for the Brier score:
```{r, "calculate EPCE weights"}
EPCE_weights <- tvEPCE(Models_folds, CVdats$testing, Tstart = tstr, Thoriz = thor)
EPCE_weights
```

The EPCE gives similar weights as the integrated Brier score with model `M1` receiving even a smaller weight. Again we see that the EPCE based on the combined cross-validate predictions is smaller than the EPCE base on the cross-validated predictions of each individual model. To combine the predictions from the five joint models we considered in this example, we need first to fit them in the original dataset:
...
We will combine the cumulative risk predictions for Patient 8 using the EPCE weights. We achieve that using the `predict()` method for objects of class `"jmList"`; we also nned to provide the `weights` argument. The rest of the arguments are the same as in the `predict()` method for `"jm"` objects (see also the Dynamic Predictions vignette):
