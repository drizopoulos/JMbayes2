...
which differs from a default call to `jm()` by the addition of the `functional_forms` argument specifying that we want an "interaction" between the marker's value and each transition, which translates into a separate association parameter for the longitudinal marker and each transition, and that we assume a Weibull baseline hazard function per transition by specifying the `base_hazard` argument.
