...
If we want to include the time-varying slope of the transformed linear predictor, we also have the `Dexpit()` and `Dexp()` functions available. As an example, we extend `jointFit2` by including the derivative of the $\expit()$ transformation:
```{r}
...
The call to `Dexpit(slope(hepatomegaly))` is internally transformed to `Dexpit(value(hepatomegaly)):slope(hepatomegaly)`, which calculates the derivative of the $\expit()$ evaluated at the linear predictor times the derivative of the linear predictor. This is because $$\frac{d}{dt} \expit\{\eta(t)\} = \expit\{\eta(t)\} \, \Bigl [ 1 - \expit\{\eta(t)\} \Bigr ] \times \frac{d}{dt}\eta(t)$$
