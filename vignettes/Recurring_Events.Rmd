...
In **JMbayes2** is now possible to fit joint models with a recurrent event process. Recurrent events are correlated events that may occur more than once over the follow-up period for a given subject. Compared to previously proposed joint models for recurrent events, we allow, for the first time, the accommodation of multiple longitudinal markers with different distributions. 

Our implementation allows the specification of various functional forms to link these markers with the risk of recurrent and terminal events. Furthermore, it enables the risk intervals to be defined in terms of the *gap* or *calendar* timescales. The two timescales use a different zero-time reference. The *calendar* uses the study entry, while the *gap* uses the end of the previous event (Figure 1). *Gap* assumes a renewal after each event and resets the time to zero.
...
A joint model with $p$ longitudinal markers, a terminal event process, and a recurrent event process can be described as follows,
...
where $i=1,\dots,\text{n subjects}$. The longitudinal processes are expressed through linear mixed-effects models, while the terminal and recurrence processes are described through proportional hazard models. The multiple processes are related via a latent structure of random effects, highlighted by the same color. $\mathcal{f}_{R_j}\left\{\eta_{j_i}(t)\right\}$ and $\mathcal{f}_{R_j}\left\{\eta_{j_i}(t)\right\}$ describe the functional forms that links the longitudinal marker $j$ with the risk of the recurrent and terminal events, respectively. The frailty $b_{F_i}$ is a random effect that accounts for the correlation intra-subjects due to unobserved individual-level characteristics.
...
For the purpose of example, we simulate data from a joint model with three outcomes: one longitudinal outcome, one terminal failure-time, and one recurrent failure-time. We assume that the underlying value of the longitudinal outcome is associated with both risk models and use the *gap* timescale. The reader can easily extend this example to accommodate multiple longitudinal markers with other forms of association, exclude the terminal event, or use a different timescale.
...
gen_data <- function(){
  ...
  for(i in seq_along(ter$id)) {
    ...
    while(!is.na(tstart) & tstart < ter$time[i]) {
      ...
      dur <- 0# runif(1, 0, 0.05) # recurrent event duration
...
We now have three data frames, each one reporting to a different outcome. To be able to fit the joint model the user needs to organize their failure-time data in the counting process formulation by combining the data for the terminal and recurrent events. Using then a strata variable to distinguish between both. To make this task easier we made available the function `rc_setup`. 
...
```{r trf_data3, echo = FALSE, fig.width = 5.5, echo = FALSE, fig.cap = "<b>Figure 3</b> Visual representation of the failure-time data during the follow-up for subject 1. The horizontal black line denotes risk periods, while the blue line non-risk periods. 'R' and 'T' represent a recurrent and terminal event, respectively."}

data <- cox_data[cox_data$id == 1,]

col_seg1 <- 1
col_seg2 <- "#002D6D" # EMC dark blue
lwd_seg1 <- 1
lwd_seg2 <- 10
...
for(i in unique(data$id)) {
  data_temp <- data[data$strata == 1 & data$id == i, ]
...
Then, the `survival::coxph()` to fit a stratified Cox model using the transformed data.
...
These models then serve as arguments in our `jm()` function. The user specifies the desired functional forms for the mixed model in each relative-risk model. And with the recurrent argument specifies the desired timescale.
