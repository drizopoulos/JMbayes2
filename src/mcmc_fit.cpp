...
List mcmc_cpp (List model_data, List model_info, List initial_values,
  ...
  vec log_weights = log(as<vec>(model_data["weights"]));
  uvec id_H = as<uvec>(model_data["id_H"]) - 1;
  uvec id_H_ = as<uvec>(model_data["id_H_"]) - 1;
  uvec id_h = as<uvec>(model_data["id_h"]) - 1;
  uvec id_h2 = as<uvec>(model_data["id_h2"]) - 1;
  uvec id_h_ = as<uvec>(model_data["id_h_"]) - 1;
  uvec intgr_ind = as<uvec>(model_data["intgr_ind"]);
  bool intgr = as<bool>(model_data["intgr"]);
  uvec id_H_fast = create_fast_ind(id_H + 1);
  uvec id_h_fast = create_fast_ind(id_h + 1);
  if (intgr) {
      id_h_fast = create_fast_ind(id_h_ + 1);
  }
  ...
  for (uword it = 0; it < n_iter; ++it) {
    ...
    for (uword j = 0; j < n_strata; ++j) {
        ...
        if (penalized_bs_gammas.at(j)) {
            ...
            double quad = arma::dot(bs_gammas_j, Tau_bs_gammas.at(j) * bs_gammas_j);
    ...
    if (recurrent) {
      ...
      update_sigmaF(sigmaF, logLik_surv,
                    ...
                    WlongH2_alphas, log_Pwk, log_Pwk2, log_weights, id_h2, intgr_ind, intgr,
                    id_H_fast, id_h_fast,
    ...
    ////////////////////////////////////////////////////////////////////////
        ...
        update_b(b, b_mat, eta, logLik_long, logLik_surv, logLik_re,
             ...
             Wh_gammas, WH2_gammas, log_Pwk, log_Pwk2, log_weights,
             id_h2, intgr_ind, intgr,
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        update_betas(betas, res_betas, acceptance_betas, scale_betas, eta,
                 ...
                 WH_gammas, Wh_gammas, WH2_gammas,
                 log_Pwk, log_Pwk2, log_weights, id_h2, intgr_ind, intgr,
                 id_H_fast, id_h_fast, which_event,
    ...
    if (recurrent) res_logLik.row(it) += trans(logLik_frailty);
  }
  if (save_random_effects) {
  ...
  return List::create(
    Named("mcmc") = List::create(
      ...
      Named("sigmaF") = res_sigmaF.rows(n_burnin, n_iter - 1)
    ),
    Named("acc_rate") = List::create(
      ...
      Named("sigmaF") = mean(acceptance_sigmaF.rows(n_burnin, n_iter - 1))
    ),
...
arma::vec logLik_jm (List thetas, List model_data, List model_info,
  ...
  vec out =
    logLik_jm_stripped(
      ...
      FunForms, Funs_FunForms, id_H_, id_h, log_Pwk, log_Pwk2, log_weights,
      id_h2, intgr_ind, intgr,
      id_H_fast, id_h_fast, which_event, which_right_event, which_left,
      which_interval,
...
arma::mat hSfun (const List &Data, const List &MCMC) {
    ...
    ///////////////////////////
    field<vec> betas_it(betas.n_elem);
    mat out(W0_h.n_rows, n_samples, fill::zeros);
    for (uword it = 0; it < n_samples; ++it) {
        field<mat> b_it = mat2field(b.slice(it), ind_RE);
        vec bs_gammas_it = bs_gammas.col(it);
        vec gammas_it = gammas.col(it);
        ...
        mat Wlong_h =
            calculate_Wlong(X_h, Z_h, U_h, Wlong_bar, Wlong_sds, betas_it, b_it,
                            id_h, FunForms, Funs_FunForms);
        vec Wlongh_alphas = Wlong_h * alphas_it;
        out.col(it) = W0h_bs_gammas + Wh_gammas + Wlongh_alphas;
    }
    return out;
}

// [[Rcpp::export]]
arma::mat variogram_cpp(const arma::field<arma::vec> &y,
                        const arma::field<arma::vec> &times) {
    uword n = y.n_elem;
    uvec Nis(n);
    for (uword i = 0; i < n; ++i) {
